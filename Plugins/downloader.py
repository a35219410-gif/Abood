...
async def dl_handler(c,m:Message):
    ...
    if text.startswith("يوت "):
        ...
        try:
            res=VideosSearch(q,limit=4).result().get("result",[])
            if not res: return await rep.edit(f"{k} ما وجدت نتائج")
            kb=[[InlineKeyboardButton(r.get("title","")[:40],callback_data=f"{m.from_user.id}GET{r.get('id','')}")] for r in res]
